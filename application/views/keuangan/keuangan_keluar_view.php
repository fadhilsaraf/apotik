        ...
        <div class="w3-container">
            <center>
              <h1 style="font-size:20pt">Keuangan Keluar</h1>
            </center>
            <br />
            <br />
            <br />
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({
        ...
        "ajax": {
            "url": "<?php echo site_url('keuangan/mengambilKeuanganKeluar')?>",
            "type": "POST"
        },
...
function setujui_keuangan(id, setujui){

  if(confirm('Apakah anda yakin untuk '+setujui+' ?'))
  {
      // ajax delete data to database
      $.ajax({
          url : "<?php echo site_url('keuangan/setujuiKeuangan')?>/"+id+"/"+setujui,
          type: "POST",
          dataType: "JSON",
    ...
    function detail_keuangan($id){
     ...
     $('#detail > tbody').html('');
     $.getJSON("<?php echo site_url('keuangan/detailKeuanganKeluar')?>/"+$id,
        function (data) {
            $.each(data, function (key, value) {           
            $('#detail > tbody').append("<tr><td>"+value.alat_transaksi+"</td><td>"+value.jumlah_uang.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")+"</td><td>"+value.bank_lawan_transaksi+"</td><td>"+value.nomor_rekening_bank_lawan_transaksi+"</td>");
        })
    })
    ...
    $('.modal-title').text('Detail keluar'); // Set Title to Bootstrap modal title
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body form">
                ...
                <thead>
                    <tr>
                        <th>Alat Transaksi</th>
                        <th>Jumlah Uang (Rp.)</th>
                        <th>Kartu Lawan Transaksi</th>
                        <th>Nomor Rekening Bank Lawan Transaksi</th>
                    </tr>
                </thead>
                <tbody>
                </tbody>
