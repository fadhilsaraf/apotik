        ...
        <div class="w3-container">
            <center>
              <h1 style="font-size:20pt">Keuangan Masuk</h1>
            </center>
            <br />
            <br />
            <br />
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({
        ...
        "ajax": {
            "url": "<?php echo site_url('keuangan/mengambilKeuanganMasuk')?>",
            "type": "POST"
        },
...
function setujui_keuangan(id, setujui){
  ...
  if(confirm('Apakah anda yakin untuk '+setujui+' ?'))
  {
    ...
    function detail_keuangan($id){
     ...
     $('#detail > tbody').html('');
     $.getJSON("<?php echo site_url('keuangan/detailKeuanganMasuk')?>/"+$id,
        function (data) {
            $.each(data, function (key, value) {           
            $('#detail > tbody').append("<tr><td>"+value.alat_transaksi+"</td><td>"+value.jumlah_uang.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")+"</td><td>"+value.kartu+"</td><td>"+value.nomor_kartu+"</td><td>"+value.nomor_transaksi+"</td></tr>");
        })
    })
